...
export const HomePhase2 = () => {
    ...
    const filteredMovies = movies.filter(movie => {
        if (filter === 'all') return movie.status === 'approved' || movie.status === 'selected';
            ...
            <header className="px-6 py-20">
                <div className="mx-auto max-w-7xl">
                    <div className="inline-flex items-center gap-3 rounded-full border border-white/20 bg-white/10 px-4 py-2 backdrop-blur mb-6">
                        ...
                        <span className="text-xs font-semibold text-white/80">Sélection Officielle</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                        ...
                        >
                            Tous ({movies.filter(m => m.status === 'approved' || m.status === 'selected').length})
                        </button>
                        <button
                        ...
                        >
                            Sélectionnés ({movies.filter(m => m.status === 'selected').length})
                        </button>
                    </div>
                </div>
            </header>
                ...
                <div className="mx-auto max-w-7xl">
                    {filteredMovies.length === 0 ? (
                        <div className="text-center py-20">
                            ...
                            <h3 className="text-xl font-bold text-white/80 mb-2">Aucun film pour le moment</h3>
                            <p className="text-white/60">Les films approuvés apparaîtront ici.</p>
                        </div>
                    ) : (
    ...
    return (
        ...
        >
            ...
            <div className="relative aspect-video bg-gradient-to-br from-violet-500/20 to-fuchsia-500/20 overflow-hidden">
                ...
                {/* Status badge */}
                <div className={`absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-bold border backdrop-blur ${statusColors[movie.status]}`}>
        ...
        >
            ...
            >
                ...
                <div className="p-6 md:p-8">
                        ...
                        <div>
                            ...
                            {movie.filmmaker_name && (
                                <p className="text-white/70">
                                    Réalisé par <span className="font-semibold text-white">{movie.filmmaker_name}</span>
                                </p>
                            )}
                        </div>
                    </div>
                    ...
                    {movie.synopsis && (
                        <div className="mb-4">
                            <h3 className="text-sm font-semibold text-white/60 uppercase tracking-wider mb-2">Synopsis</h3>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-white/10">
                        ...
                        {movie.production_year && (
                            <div>
                                ...
                                <p className="text-white font-semibold">{movie.production_year}</p>
                            </div>
                        )}
                        {movie.country && (
                            <div>
