...
export default function PartnersTable({ partners, loading, deletingId, onDelete }) {
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full text-left text-xs">
        ...
        <tbody className="divide-y divide-slate-800/80 text-[13px] text-slate-100">
          {loading && <LoadingRow colSpan={4} message="Chargement des partenaires..." />}
          {!loading && partners.length === 0 && <EmptyRow colSpan={4} message="Aucun partenaire." />}
          {!loading && partners.map((p) => (
            <tr key={p.id}>
              <td className="px-2 py-2">{p.name}</td>
              <td className="px-2 py-2 text-xs text-brand-muted">
                {p.website_url ? <a href={p.website_url} target="_blank" rel="noreferrer" className="underline decoration-dotted">{p.website_url}</a> : "—"}
              </td>
              <td className="px-2 py-2 text-xs text-brand-muted max-w-xs truncate">{p.description || "—"}</td>
              <td className="px-2 py-2 text-right text-[11px]">
                <button type="button" disabled={deletingId === p.id} onClick={() => onDelete(p.id)}
