...
export default function VideosDistribution({ currentAdmin }) {
  ...
  const handleDistribute = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || data.message || "Impossible de lancer la répartition automatique.");
      setResult(data);
    } catch (err) {
    ...
    <div className="space-y-4">
      ...
      {!isSuperAdmin && (
        <p className="rounded-md border border-amber-500/60 bg-amber-950/40 px-3 py-2 text-xs text-amber-100">
          Seul un compte <span className="font-semibold">super_admin</span> peut lancer la répartition.
        </p>
      )}
      <SectionCard title="Répartition">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div className="space-y-2">
            <label className="block text-[11px] font-medium text-brand-muted">Nombre minimum d&apos;admins par vidéo</label>
            <input
              type="number"
              min={1}
              value={minReviewers}
            ...
            />
            <p className="text-[11px] text-brand-muted max-w-md">Laissez à 2 pour que chaque vidéo soit vue par au moins deux admins.</p>
          </div>
          <button
            type="button"
        ...
        {result && (
          <div className="mt-3 rounded-md border border-emerald-500/40 bg-emerald-950/30 px-3 py-3 text-xs text-emerald-100 space-y-1">
            <p className="font-semibold text-emerald-200">Répartition effectuée avec succès.</p>
            <p>Films concernés : <span className="font-mono">{result.moviesCount ?? "—"}</span></p>
            <p>Nouvelles assignations : <span className="font-mono">{result.assignmentsCreated ?? "0"}</span></p>
            <p>Minimum admins/vidéo : <span className="font-mono">{result.reviewersRequired ?? minReviewers}</span></p>
          </div>
        )}
      </SectionCard>
    </div>
