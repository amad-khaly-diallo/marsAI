...
export default function AdminFilmGallery({ movies, loading, error, onReload }) {
  ...
  const currentMovie =
    activeIndex != null && hasMovies && activeIndex >= 0 && activeIndex < movies.length
      ? movies[activeIndex]
      : null;
  ...
  useEffect(() => {
    ...
    if (!reviewsCache[currentMovie.id]) {
  ...
  const handleToggleWinner = async (movie) => {
    if (!movie) return;
    const makeWinner = !movie.is_winner;
      ...
      {!loading && hasMovies && (
        <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 xl:grid-cols-4">
          {movies.map((movie, index) => {
            ...
            const isWinner = movie.is_winner;
