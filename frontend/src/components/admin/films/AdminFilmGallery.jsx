...
export default function AdminFilmGallery({ movies, loading, error, onReload }) {
  ...
  const winnersCount = useMemo(() => (movies || []).filter((m) => m.is_winner).length, [movies]);
  const currentMovie =
    activeIndex != null && hasMovies && activeIndex >= 0 && activeIndex < movies.length ? movies[activeIndex] : null;
  const currentReviews = currentMovie && reviewsCache[currentMovie.id] ? reviewsCache[currentMovie.id] : [];

  const fetchReviews = async (id) => {
    setLoadingReviewId(id);
    setReviewsError(null);
    try {
      const res = await fetch(`/api/admin/films/${id}/reviews`, { method: "GET", credentials: "include" });
      const data = await res.json().catch(() => []);
      if (!res.ok) throw new Error(data.error || data.message || "Erreur de chargement des avis.");
  ...
  const handleToggleWinner = async (movie) => {
    ...
    try {
      ...
      setWinnerError(err.message || "Impossible de mettre à jour le statut gagnant.");
    } finally {
  ...
  return (
    <div className="space-y-3">
      {error && <p className="mb-2 rounded-md border border-red-500/60 bg-red-950/40 px-3 py-2 text-xs text-red-200">{error}</p>}
      {loading && <div className="py-8 text-center text-sm text-brand-muted">Chargement des films...</div>}
      {!loading && !hasMovies && !error && <div className="py-8 text-center text-sm text-brand-muted">Aucun film à afficher pour le moment.</div>}
      {!loading && hasMovies && (
            ...
            return (
              ...
              >
                <div className="relative aspect-video w-full overflow-hidden bg-black">
                  {thumbUrl ? (
                    <>
                      <img src={thumbUrl} alt={movie.original_title || "Miniature du film"} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" />
                    ...
                    <div className="flex h-full items-center justify-center text-[11px] text-gray-500">Pas de miniature</div>
                  )}
                  {movie.is_winner && (
                    <div className="absolute left-2 top-2 rounded-full bg-amber-500/90 px-2 py-0.5 text-[10px] font-semibold text-slate-900 shadow">Gagnant</div>
                  )}
                </div>
                <div className="flex flex-1 flex-col gap-2 p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 space-y-0.5">
                      <p className="truncate text-sm font-semibold text-slate-50">{movie.original_title || "Sans titre"}</p>
