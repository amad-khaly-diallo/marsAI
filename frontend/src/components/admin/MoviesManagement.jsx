...
export default function MoviesManagement() {
  ...
  const deleteMovie = async (id) => {
    ...
    try {
      const res = await fetch(`/api/admin/films/${id}`, { method: "DELETE", credentials: "include" });
  ...
  return (
    <div className="space-y-4 relative">
      <header className="space-y-1">
        ...
        <p className="text-sm text-brand-muted">Gérez les soumissions YouTube et les statuts.</p>
      </header>
          ...
          <>
            <div className="grid grid-cols-1 gap-6 md:hidden">
              {movies.map((movie) => {
                ...
                return (
                  <div key={movie.id} className="bg-slate-900/50 border border-slate-700/50 rounded-xl overflow-hidden shadow-lg relative">
                    
                    <div className="relative w-full aspect-video bg-black">
                      {embedUrl ? (
                        <iframe src={embedUrl} className="w-full h-full" allowFullScreen title={movie.original_title} frameBorder="0" />
                      ) : (
                        <div className="flex h-full items-center justify-center text-xs text-gray-500">Pas de lien YouTube</div>
                      )}
                    </div>
                    ...
                    <div className="p-4 space-y-3">
                      <div className="flex justify-between items-start">
                        <div>
                          <h3 className="text-base font-bold text-slate-100">{movie.original_title || "Sans titre"}</h3>
                          <p className="text-xs text-brand-muted">{movie.filmmaker ? `${movie.filmmaker.first_name} ${movie.filmmaker.last_name}` : "—"}</p>
                        </div>
                        <span className={`inline-flex rounded-full px-2 py-1 text-[10px] border ${
              ...
              <table className="min-w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-800/80 text-[13px] text-slate-100">
                  {movies.map((movie) => {
                    const embedUrl = getYouTubeEmbed(movie.youtube_url);
                    return (
                      <tr key={movie.id} className="hover:bg-slate-800/30 transition-colors relative">
                        <td className="px-2 py-3 align-top">
                          <div className="w-60 h-34 bg-black rounded-xl overflow-hidden border border-slate-700 shadow-sm relative">
                            {embedUrl ? (
                              <iframe src={embedUrl} className="w-full h-full" allowFullScreen title={movie.original_title} frameBorder="0" />
                            ) : (
                              <div className="flex h-full items-center justify-center text-[10px] text-gray-500">Pas de lien YouTube</div>
                            )}
                          </div>
                        </td>
                        <td className="px-2 py-3 align-middle">
                          <div className="font-bold text-sm text-slate-100">{movie.original_title || "Sans titre"}</div>
                          <div className="text-xs text-brand-muted mt-1">{movie.filmmaker ? `${movie.filmmaker.first_name} ${movie.filmmaker.last_name}` : "—"}</div>
                          <div className="text-xs text-slate-500 mt-1 flex items-center gap-1">
                            <span>⏱</span> {movie.duration ? `${movie.duration} min` : "N/A"}
                          </div>
                        </td>
                        <td className="px-2 py-3 text-center align-middle font-mono text-gray-400">{movie.views || 0}</td>
                        ...
                        <td className="px-2 py-3 text-right align-middle relative">
                          ...
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => updateStatus(movie.id, "approved")} title="Approuver" className="p-2 text-emerald-400 hover:bg-emerald-500/10 rounded-md transition-colors">
                            ...
                            <button onClick={() => updateStatus(movie.id, "rejected")} title="Rejeter" className="p-2 text-red-400 hover:bg-red-500/10 rounded-md transition-colors">
                            ...
                            <button onClick={() => updateStatus(movie.id, "selected")} title="Sélectionner" className="p-2 text-brand-primary hover:bg-brand-primary/10 rounded-md transition-colors">
                              <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="currentColor" stroke="none"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                            </button>
                            
                            <div className="w-px h-5 bg-slate-700 mx-1"></div>
                            <button onClick={() => deleteMovie(movie.id)} title="Supprimer" className="p-2 text-slate-400 hover:text-red-400 hover:bg-red-500/10 rounded-md transition-colors">
