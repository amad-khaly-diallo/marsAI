...
export default function GreenFlagGallery() {
  ...
  const flatFavorites = useMemo(
    () => groups.flatMap((g) => (g.movies || []).map((m) => ({ admin: g, movie: m }))),
    [groups],
  );
  const totalPages = Math.max(1, Math.ceil((flatFavorites.length || 0) / PAGE_SIZE));
  ...
  const paginatedGroups = useMemo(() => {
    ...
    pageItems.forEach(({ admin: a, movie }) => {
      if (!byAdmin.has(a.admin_id)) byAdmin.set(a.admin_id, { ...a, movies: [] });
  ...
  const handleSelect = async () => {
    ...
    try {
      ...
      setGroups((prev) =>
        prev.map((g) => ({ ...g, movies: g.movies.map((m) => (m.id === movieId ? { ...m, status: "selected" } : m)) })),
      );
  ...
  return (
    <div className="space-y-6">
      ...
      {loading && <div className="py-12 text-center text-brand-muted">Chargement des favoris du jury...</div>}
      ...
      {!loading && hasData && paginatedGroups.map((group) => (
        <section key={group.admin_id} className="space-y-3">
          <div className="flex items-center justify-between gap-2">
            <div>
              <h3 className="text-sm font-semibold text-slate-100">{group.first_name} {group.last_name}</h3>
              <p className="text-[11px] text-brand-muted">{group.email}</p>
            </div>
            <span className="text-[11px] text-brand-muted">{group.movies.length} film(s) en green flag</span>
          </div>
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
            {group.movies.map((movie) => (
              <GreenFlagCard key={movie.id} group={group} movie={movie} onOpen={setActiveFavorite} />
            ))}
          </div>
        </section>
      ))}
      {hasData && (
        <div className="flex items-center justify-center gap-3 pt-2 text-[11px] text-brand-muted">
          ...
          <span>Page <span className="font-semibold text-slate-100">{currentPage}</span> / {totalPages}</span>
      ...
      <GreenFlagModal favorite={activeFavorite} onClose={() => setActiveFavorite(null)} onSelect={handleSelect} selecting={!!selectingId} />
    </div>
  );
}
