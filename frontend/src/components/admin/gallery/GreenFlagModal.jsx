...
export default function GreenFlagModal({ favorite, onClose, onSelect, selecting }) {
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-brand-muted">
              Jury : <span className="font-semibold text-slate-100">{admin.first_name} {admin.last_name}</span> ({admin.email})
            </p>
          </div>
          <button type="button" onClick={onClose} className="rounded-full bg-slate-900/80 px-2 py-1 text-slate-400 hover:bg-slate-800 hover:text-slate-100 text-xs">
            Fermer
          </button>
        </div>
        <div className="flex flex-col gap-4 md:flex-row">
          ...
          <div className="flex flex-1 flex-col gap-3">
            <div className="space-y-1">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-brand-muted">Réalisateur</p>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-200 line-clamp-6">{movie.synopsis_original || "Pas de synopsis disponible."}</p>
            </div>
            <div className="mt-auto flex flex-col gap-2 border-t border-slate-800 pt-3">
              <p className="text-[11px] text-brand-muted">
                Ce film est marqué <span className="font-semibold text-emerald-300">green flag</span> par ce membre du jury.
              </p>
              <button type="button" onClick={onSelect} disabled={selecting}
