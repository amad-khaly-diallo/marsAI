...
export default function JuryTable({
  members,
  loading,
  deletingId,
  onDelete,
          ...
          {!loading &&
            members.map((m) => (
              <tr key={m.id}>
                ...
                <td className="px-2 py-2 text-right text-[11px] space-x-2">
                  <button
                    type="button"
                    onClick={() => onEdit && onEdit(m)}
                    className="rounded-full bg-slate-700 px-2 py-0.5 text-[11px] font-semibold text-slate-100 hover:bg-slate-600"
                  >
                    Éditer
                  </button>
                  <button
