...
export default function NewslettersManagement() {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      setSendResult(typeof data.sent === "number" ? `Newsletter envoyée à ${data.sent} abonné(s).` : "Newsletter envoyée.");
      setSubject("");
      setBody("");
    } catch (err) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-[2fr,1.2fr]">
        <SectionCard
          ...
          action={<span className="text-[11px] text-brand-muted">Basé sur les abonnements newsletter des réalisateurs.</span>}
        >
        ...
        <SectionCard
          ...
          action={null}
        >
          <p className="mt-1 text-[11px] text-brand-muted">Un email sera envoyé à tous les abonnés.</p>
          <SendNewsletterForm
            subject={subject}
