...
import { StatCard, AnalyticsBar } from "../home/HomeComponents";
...
export function StatsSection() {
  ...
  // Afficher tous les chiffres clés (TOUS) sans rotation
  const currentGenre = FILM_GENRES[0];
  const filteredStats = FILM_GENRES[0].stats;
  const filteredAnalytics = FILM_GENRES[0].analytics;
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-6">
          ...
          <p className="mt-2 max-w-2xl text-xs leading-5 text-white/70">
            Une co-création de l'école du numérique La Plateforme et du Mobile Film Festival. En nous basant sur les résultats obtenus par les événements et sélections organisés.
          </p>
        </div>
...
function CameraButton({ statsRevealed, statsFlash, onToggle, selectedGenre, selectedGenreIndex, onRotateGenre, isRotating }) {
  const FILM_GENRES_LOCAL = [
    { name: "TOUS", icon: "⭐", color: "from-violet-500 to-fuchsia-600" },
  ...
  return (
    <div className="flex-shrink-0 flex flex-col items-center gap-2 relative w-full lg:w-auto px-4 lg:px-0">
      {/* Roulette supprimée : projecteur uniquement */}
