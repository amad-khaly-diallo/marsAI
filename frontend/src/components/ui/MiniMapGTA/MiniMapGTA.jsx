...
const GareMarker = ({ onToggle, onKeyDown }) => (
  <InteractiveMarker id="gare" onToggle={onToggle} onKeyDown={onKeyDown}>
    <rect x="50" y="190" width="8" height="8" fill="rgba(255, 150, 60, 0.95)" stroke="rgba(255, 180, 100, 1)" strokeWidth="0.6" filter="url(#shadow)" rx="1" />
    <line x1="50" y1="194" x2="58" y2="194" stroke="rgba(255, 200, 150, 0.7)" strokeWidth="0.5" />
  </InteractiveMarker>
);
...
const HopitalMarker = ({ onToggle, onKeyDown }) => (
  <InteractiveMarker id="hopital" onToggle={onToggle} onKeyDown={onKeyDown}>
    <circle cx="320" cy="240" r="5.5" fill="rgba(255, 80, 80, 0.9)" stroke="rgba(255, 150, 150, 1)" strokeWidth="0.7" filter="url(#shadow)" />
...
const MairieMarker = ({ onToggle, onKeyDown }) => (
  <InteractiveMarker id="mairie" onToggle={onToggle} onKeyDown={onKeyDown}>
    <g transform="translate(170, 210)">
      <rect x="-7" y="-6" width="14" height="10" fill="rgba(120, 200, 255, 0.95)" stroke="rgba(160, 220, 255, 1)" strokeWidth="0.7" filter="url(#shadow)" />
      <rect x="-6" y="-2" width="2" height="6" fill="rgba(255, 250, 200, 0.9)" rx="0.3" />
      <rect x="-2" y="-2" width="2" height="6" fill="rgba(255, 250, 200, 0.9)" rx="0.3" />
      <rect x="2" y="-2" width="2" height="6" fill="rgba(255, 250, 200, 0.9)" rx="0.3" />
      <rect x="5" y="-2" width="1" height="6" fill="rgba(255, 250, 200, 0.9)" rx="0.3" />
      <polygon points="-7,-6 0,-11 7,-6" fill="rgba(200, 50, 50, 0.8)" stroke="rgba(220, 80, 80, 0.9)" strokeWidth="0.5" />
    </g>
  </InteractiveMarker>
);
...
const FestivalMarker = ({ onToggle, onKeyDown }) => (
  <InteractiveMarker id="festival" onToggle={onToggle} onKeyDown={onKeyDown}>
    <g transform="translate(200, 150)">
      <circle cx="0" cy="0" r="15" fill="none" stroke="rgba(168, 85, 247, 0.5)" strokeWidth="2" />
      <circle cx="0" cy="0" r="8" fill="none" stroke="rgba(168, 85, 247, 0.3)" strokeWidth="1" />
      <polygon
      ...
      <text x="0" y="4" fontSize="5" fill="white" fontWeight="bold" textAnchor="middle" fontFamily="Arial">MARS</text>
    </g>
  </InteractiveMarker>
);
...
const MainRoad = ({ x1, y1, x2, y2, strokeWidth = 5 }) => (
  <>
    <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="#d8d8d8" strokeWidth={strokeWidth} opacity="0.85" strokeLinecap="round" />
    <line x1={x1} y1={y1} x2={x2} y2={y2} stroke="#a0d8ff" strokeWidth={strokeWidth / 2.5} opacity="0.4" strokeLinecap="round" />
  </>
);
    ...
    <div className="relative w-full h-full">
      <div className="relative w-full h-full">
        ...
        >
          ...
          <rect x="800" y="80" width="200" height="320" fill="url(#waterGradient)" />

          {/* Eau - Baie (bas) */}
          <polygon points="0,350 375,340 500,360 625,345 1000,360 1000,400 0,400" fill="url(#waterGradient)" opacity="0.6" />

          {/* Parcs/Espaces verts - Parc Borely */}
          <rect x="75" y="80" width="125" height="125" fill="url(#parkGradient)" filter="url(#shadow)" />
          
          {/* Parc Chanot */}
          <circle cx="700" cy="120" r="75" fill="url(#parkGradient)" filter="url(#shadow)" />
          
          {/* Jardin Pierre Puget */}
          <polygon points="450,300 575,290 600,340 425,345" fill="url(#parkGradient)" />

          {/* Blocs urbains (remplissage) */}
          <rect x="225" y="95" width="60" height="40" fill="rgba(120, 120, 120, 0.25)" />
          <rect x="337" y="105" width="50" height="30" fill="rgba(120, 120, 120, 0.22)" />
          <rect x="500" y="105" width="45" height="35" fill="rgba(120, 120, 120, 0.22)" />
          <rect x="625" y="165" width="55" height="40" fill="rgba(120, 120, 120, 0.2)" />
          <rect x="175" y="230" width="65" height="45" fill="rgba(120, 120, 120, 0.2)" />
          <rect x="375" y="230" width="70" height="40" fill="rgba(120, 120, 120, 0.2)" />
          <rect x="525" y="230" width="55" height="35" fill="rgba(120, 120, 120, 0.2)" />
          <rect x="650" y="290" width="60" height="45" fill="rgba(120, 120, 120, 0.2)" />
          ...
          {VERTICAL_ROADS.map((road, idx) => (
            <line key={`vroad-${idx}`} x1={road.x} y1={road.y1} x2={road.x} y2={road.y2} stroke="#8a8a8a" strokeWidth="1.2" opacity={road.opacity} />
          ))}

          {/* Lignes horizontales */}
          {HORIZONTAL_ROADS.map((road, idx) => (
            <line key={`hroad-${idx}`} x1={road.x1} y1={road.y} x2={road.x2} y2={road.y} stroke="#8a8a8a" strokeWidth="1.2" opacity={road.opacity} />
          ))}

          {/* ===== ROUTES COURBES (LITTORAL ET ZONES ORGANIQUES) ===== */}
          {CURVED_ROADS.map((path, idx) => (
            <path key={`curved-${idx}`} d={path} stroke="#8a8a8a" strokeWidth="1.5" fill="none" opacity="0.6" />
          ))}

          {/* ===== RONDS-POINTS (intersections) ===== */}
          {ROUNDABOUTS.map((point, idx) => (
            <circle key={`roundabout-${idx}`} cx={point.cx} cy={point.cy} r={point.r} fill="none" stroke="#a5a5a5" strokeWidth="1" opacity="0.7" />
          ))}
          ...
          <g>
            ...
            <polygon points="320,38 316,50 324,50" fill="rgba(255, 200, 80, 1)" stroke="rgba(255, 220, 150, 1)" strokeWidth="0.6" filter="url(#glow)" />
            <circle cx="320" cy="42" r="1.5" fill="rgba(255, 240, 180, 1)" />
          </g>
          
          {/* Vieux Port (rectangle bleu amélioré) */}
          <rect x="100" y="50" width="60" height="30" fill="rgba(30, 150, 220, 0.6)" stroke="rgba(100, 200, 255, 0.8)" strokeWidth="1.2" filter="url(#shadow)" rx="2" />
          <text x="125" y="70" fontSize="7" fill="rgba(200, 240, 255, 0.9)" fontWeight="bold" textAnchor="middle" fontFamily="Arial">PORT</text>
          
          {/* Fort Saint-Jean (petit carré renforcé) */}
          <rect x="95" y="75" width="12" height="12" fill="none" stroke="rgba(255, 150, 80, 0.85)" strokeWidth="1.2" filter="url(#shadow)" rx="1" />
          <line x1="98" y1="78" x2="104" y2="84" stroke="rgba(255, 150, 80, 0.7)" strokeWidth="0.5" />

          {/* Docks (petits quais améliorés) */}
          <g opacity="0.8">
            <rect x="105" y="40" width="8" height="6" fill="rgba(200, 220, 240, 0.7)" stroke="rgba(180, 200, 220, 0.6)" strokeWidth="0.3" rx="1" />
            <rect x="118" y="40" width="8" height="6" fill="rgba(200, 220, 240, 0.7)" stroke="rgba(180, 200, 220, 0.6)" strokeWidth="0.3" rx="1" />
            <rect x="131" y="40" width="8" height="6" fill="rgba(200, 220, 240, 0.7)" stroke="rgba(180, 200, 220, 0.6)" strokeWidth="0.3" rx="1" />
            <rect x="144" y="40" width="8" height="6" fill="rgba(200, 220, 240, 0.7)" stroke="rgba(180, 200, 220, 0.6)" strokeWidth="0.3" rx="1" />
          </g>
