...
const Jury = () => {
  ...
  useEffect(() => {
    fetch('/api/jury')
      .then(res => res.json())
      .then(data => { setJuryMembers(data); setLoading(false); })
  ...
  useEffect(() => {
    if (scrollRef.current?.children[activeIndex]) {
      setTimeout(() => {
        scrollRef.current.children[activeIndex].scrollIntoView({ behavior: 'smooth', inline: 'center' });
      }, 100);
    }
  }, [activeIndex]);

  const angleStep = useMemo(() => juryMembers.length ? 360 / juryMembers.length : 0, [juryMembers]);
...
<div className="absolute inset-0 z-0 pointer-events-none">
  ...
  <div className="absolute inset-0 bg-[#070819]/50"></div>
      ...
      {/* Section mobile */}
      <nav className={`md:hidden w-full overflow-x-auto py-4 mb-2 flex items-start gap-6 px-10 z-10 ${styles.noScrollbar}`} ref={scrollRef}>
        {juryMembers.map((member, index) => (
          <JuryAvatar 
            key={member.id} member={member} index={index} activeIndex={activeIndex} 
            isMobile={true} onClick={setActiveIndex} getAssetImage={getAssetImage}
          />
        ))}
      </nav>
      ...
      <main className="relative w-full h-[480px] md:h-[550px] flex items-center justify-center z-10">
        ...
        <div className="hidden md:block absolute w-full h-full transition-transform duration-1000 ease-out" style={{ transform: `rotate(${activeIndex * -angleStep}deg)` }}>
          {juryMembers.map((member, index) => (
            <JuryAvatar 
              key={member.id} member={member} index={index} activeIndex={activeIndex} 
              hoverIndex={hoverIndex} isMobile={false} onClick={setActiveIndex}
              onMouseEnter={setHoverIndex} onMouseLeave={() => setHoverIndex(null)}
              getAssetImage={getAssetImage} angleStep={angleStep} radius={radius}
            />
          ))}
        </div>
      </main>
