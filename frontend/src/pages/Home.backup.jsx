...
import MiniMapGTA from "../components/MiniMapGTA/MiniMapGTA";

/* ---------- Helpers ---------- */

function useCountUp(target, duration = 4000, triggerOnce = true, enabled = true) {
...
function StatCard({ icon, title, subtitle, value, prefix, gradient, revealed }) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.05] p-6 backdrop-blur">
      <div className="flex items-center gap-3 mb-4">
        <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${gradient.bg} border ${gradient.border} flex items-center justify-center`}>
      ...
      <div ref={ref}>
        <div className={`text-3xl font-black text-transparent bg-clip-text ${gradient.textGradient}`}>
    ...
    <div className="flex items-center justify-between hover:bg-white/5 transition-all p-2 rounded">
      ...
      <div className="flex items-center gap-2">
        <div className="w-20 h-1 bg-black/40 rounded-full overflow-hidden">
          <div className={`h-full bg-gradient-to-r from-${colorFrom}-500 to-${colorTo}-500 rounded-full`} style={{width: `${percentage}%`}} />
        </div>
        <span className={`text-xs font-bold text-${textColor}-300 w-10 text-right`}>{percentage}%</span>
      </div>
    </div>
  );
}
...
export default function Home() {
  ...
  const statsData = [
    {
      icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 12H9m6 0a6 6 0 11-12 0 6 6 0 0112 0z" />,
      title: "Visiteurs",
    ...
    {
      icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 4v16m0 0H3m4 0h10m0 0h4m-4 0v-2a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m0 0v2a2 2 0 002 2h4a2 2 0 002-2v-2m0 0h4" />,
      title: "Films soumis",
  ...
  const analyticsData = [
    { label: "Courts", percentage: 65, colorFrom: "cyan", colorTo: "blue", textColor: "cyan" },
    { label: "Docs", percentage: 42, colorFrom: "fuchsia", colorTo: "pink", textColor: "fuchsia" },
    { label: "Expo", percentage: 28, colorFrom: "green", colorTo: "emerald", textColor: "green" },
    { label: "Anims", percentage: 55, colorFrom: "amber", colorTo: "yellow", textColor: "amber" }
  ];
            ...
            <div className="flex justify-center lg:justify-end animate-fadeIn">
              <div className="relative w-full lg:w-auto px-4 lg:px-0">
                ...
                <div className="absolute -bottom-6 lg:-bottom-8 left-1/2 -translate-x-1/2 text-xs text-white/50 font-semibold">
                  {mapMode === 'gta' ? 'Radar GTA-style' : 'Carte réelle Marseille'}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-6xl">
          <div className="mb-12">
            <div className="inline-flex items-center gap-3 rounded-full border border-white/20 bg-white/10 px-4 py-2 backdrop-blur">
              ...
              <span className="text-xs font-semibold text-white/80">Résultats & Projections</span>
            </div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-white/70">
              ...
              En nous basant sur les résultats obtenus par les événements et sélections organisés.
            </p>
          </div>
          ...
          <div className="relative max-w-6xl mx-auto">
            ...
            {statsRevealed && (
              <>
                {/* Halo principal */}
                <div className="absolute left-0 top-1/2 -translate-y-1/2 -z-10 pointer-events-none transition-all duration-700"
                  style={{
                    width: '400px',
                    height: '400px',
                ...
                />
                {/* Halo secondaire */}
                <div className="absolute inset-0 -z-10 pointer-events-none transition-all duration-700 rounded-full blur-3xl"
                  style={{
                    background: 'radial-gradient(circle at 20% 50%, rgba(147, 51, 234, 0.25) 0%, transparent 50%)',
                  }}
                />
              </>
            )}
            ...
            <div className="relative flex flex-col lg:flex-row gap-0 items-center lg:items-stretch">
              ...
              <div className="flex-shrink-0 flex flex-col items-center gap-4 relative w-full md:w-auto px-4 md:px-0">
                ...
                >
                  ...
                  <img
                    src={statsRevealed ? require('../assets/images/allumer.png') : require('../assets/images/éteint.png')}
                ...
                >
                  {/* Background glow */}
                  <div className={`absolute inset-0 rounded-full blur-md transition-all duration-500 -z-10 ${
                    statsRevealed ? 'bg-green-500/60 opacity-100' : 'bg-red-500/60 opacity-100'
                  }`} />
                  
                  {/* Text labels OFF/ON */}
                  <span className={`absolute left-1.5 text-[9px] font-extrabold transition-all duration-500 ${
                    statsRevealed ? 'text-red-200 opacity-20' : 'text-red-50 opacity-100'
                  }`}>
                    OFF
                  </span>
                  <span className={`absolute right-1.5 text-[9px] font-extrabold transition-all duration-500 ${
                  ...
                  {/* Sliding circle */}
                  <div className={`relative h-6 w-6 transform rounded-full bg-white shadow-xl transition-all duration-500 flex items-center justify-center group-hover:shadow-2xl ${
                    ...
                    <div className={`relative w-2 h-2 rounded-full transition-all duration-500 ${
                      ...
                      {statsRevealed && <div className="absolute inset-0 rounded-full bg-green-400 opacity-60 animate-ping" />}
                      {!statsRevealed && <div className="absolute inset-0 rounded-full bg-red-400 opacity-60 animate-ping" />}
                    </div>
                  </div>
                </button>
              </div>
              ...
              {statsRevealed && (
                <div className="absolute left-1/2 sm:left-40 md:left-52 top-[36%] -translate-y-1/2 md:-translate-y-0 md:top-auto md:bottom-0 w-[calc(100%-6rem)] sm:w-[calc(100%-10rem)] md:w-[calc(100%-13rem)] h-12 sm:h-16 md:h-20 md:w-12 md:h-full pointer-events-none z-20 overflow-visible"
                  style={{
                    display: 'flex',
              ...
              {statsRevealed && (
                <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 pointer-events-none transition-all duration-500 z-30"
                  style={{
                    background: 'radial-gradient(circle, rgba(255, 255, 255, 0.6) 0%, rgba(147, 51, 234, 0.3) 35%, transparent 70%)',
                    filter: 'blur(40px)',
                  }}
                />
              )}
              ...
              >
                ...
                {statsRevealed && (
                  <div className="absolute inset-0 pointer-events-none opacity-[0.03] mix-blend-overlay animate-grain"
                    style={{
                ...
                <div className="flex flex-col lg:flex-row gap-4 md:gap-8 h-full">
                  ...
                  <div className={`flex-1 transition-all duration-700 delay-150 ${
                    statsRevealed 
                      ? 'opacity-100 translate-x-0' 
                      : 'opacity-0 translate-x-8 pointer-events-none'
                  }`}
                  ...
                  >
                    <div className="relative bg-black/90 border-2 border-cyan-400/60 rounded-lg p-3 md:p-4 backdrop-blur-xl h-full flex flex-col overflow-hidden shadow-2xl shadow-cyan-500/50">
                      ...
                      <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between mb-3 md:mb-4 pb-2 border-b border-cyan-400/30 gap-2">
                        <div>
                          <h3 className="text-xs sm:text-sm font-black text-cyan-400 uppercase tracking-widest" style={{textShadow: '0 0 10px rgba(0, 255, 255, 0.8)'}}>
                            ≡ DISTRIBUTION
                          </h3>
                          <div className="text-[9px] text-magenta-400 font-mono mt-0.5">SYS.ANALYTICS_v2.1</div>
                        </div>
                        <div className="flex items-center gap-1.5 px-2 py-1 bg-cyan-500/20 border border-cyan-400 rounded">
                          <div className="w-1.5 h-1.5 bg-cyan-400 animate-pulse" style={{boxShadow: '0 0 8px rgba(0, 255, 255, 0.8)'}} />
                          <span className="text-[10px] font-bold text-cyan-300 font-mono">ACTIVE</span>
                        </div>
                      </div>
                          ...
                          <div key={index} className="group">
                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-1 sm:mb-1.5 gap-1">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] sm:text-xs font-bold text-white uppercase tracking-wide font-mono">{item.label}</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <div className="text-[9px] sm:text-xs text-cyan-400 font-mono">▓▓▓</div>
                                <span className={`text-xs sm:text-sm font-black text-${item.textColor}-400 font-mono`} style={{textShadow: `0 0 10px rgba(0, 255, 255, 0.5)`}}>
                                  {item.percentage}%
                                </span>
                              </div>
                            </div>
                      ...
                      <div className="relative z-10 mt-4 pt-3 border-t border-cyan-400/30">
                        <div className="grid grid-cols-2 gap-2">
                          <div className="bg-cyan-500/10 border border-cyan-400/40 p-2 relative overflow-hidden">
                            <div className="absolute top-0 right-0 text-cyan-400/20 text-2xl font-mono">≡</div>
                            <p className="text-[8px] text-cyan-300 uppercase tracking-wider font-mono mb-1">AVG_RATE</p>
                            <p className="text-base font-black text-cyan-400 font-mono" style={{textShadow: '0 0 10px rgba(0, 255, 255, 0.8)'}}>47.5%</p>
                          </div>
                          <div className="bg-magenta-500/10 border border-magenta-400/40 p-2 relative overflow-hidden">
                            <div className="absolute top-0 right-0 text-magenta-400/20 text-2xl font-mono">▲</div>
                            <p className="text-[8px] text-magenta-300 uppercase tracking-wider font-mono mb-1">TOP_CAT</p>
                            <p className="text-xs font-black text-magenta-400 font-mono uppercase" style={{textShadow: '0 0 10px rgba(255, 0, 255, 0.8)'}}>Courts</p>
                          </div>
                        </div>
                      </div>
                    </div>
